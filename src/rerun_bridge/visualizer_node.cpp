...
void RerunLoggerNode::_create_subscribers() {
    ...
    for (const auto& topic_info : topic_infos) {
        ...
        } else if (topic_info.datatype == "sensor_msgs/PointCloud2") {
...
ros::Subscriber RerunLoggerNode::_create_pointcloud_subscriber(const std::string& topic) {
    std::string entity_path = _resolve_entity_path(topic);
    bool lookup_transform = (_topic_to_entity_path.find(topic) == _topic_to_entity_path.end());
    ...
    return _nh.subscribe<sensor_msgs::PointCloud2>(
        topic,
        10,
        [&, entity_path, lookup_transform](const sensor_msgs::PointCloud2::ConstPtr& msg) {
            if (!_root_frame.empty() && lookup_transform) {
                try {
                    auto transform = _tf_buffer.lookupTransform(
                        _root_frame,
            ...
            log_point_cloud(_rec, entity_path, msg);
        }
    );
}
